...
boot_infosec() {
      ...
      mkdir -p /run/media/cryptroot
      cryptsetup -d /run/media/hda2/key/keyfile open /dev/hda3 cryptroot
      mount /dev/mapper/cryptroot /run/media/cryptroot
  
      # Prepare for switch root
      umount /run/media/hda2
      mount --move /proc /run/media/cryptroot/proc
      mount --move /sys /run/media/cryptroot/sys
      mount --move /dev /run/media/cryptroot/dev
  
      # Switch root
      exec switch_root -c /dev/console /run/media/cryptroot /sbin/init
}
# End
...
# Execute boot_infosec only normal booting
if [ "${LABEL}" = "SK Infosec" ]; then
    boot_infosec
fi
# End
