...
# Remark logic formatting root partition
# To apply dm-crypt
...
# Generating key for LUKS
dd if=/dev/urandom of=keyfile bs=1c count=32

# Initialize LUKS partition for root filesystem
echo Y | cryptsetup luksFormat ${rootfs} keyfile
cryptsetup -d keyfile open ${rootfs} cryptroot
...
# Closing encrypted root partition
...
menuentry "SK Infosec" {
    search --no-floppy --fs-uuid $boot_uuid --set root
    linux /vmlinuz root=PARTUUID=$root_part_uuid $rootwait LABEL="SK Infosec" rw $5 $3 $4 quiet
    initrd /initrd
}
_EOF
    chmod 0444 $GRUBCFG
...
# Copying initramfs image
cp /run/media/$1/initrd /boot/
# End 

# Copying keyfile to boot partition
mkdir -p /boot/key
cp -f keyfile /boot/key/keyfile
